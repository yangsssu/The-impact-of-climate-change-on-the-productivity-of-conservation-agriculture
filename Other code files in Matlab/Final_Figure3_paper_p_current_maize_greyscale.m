...
for m=1:360
    for n=1:720
        if (maize_mask2(m,n)== 0 || maize_Ipsl_cm5a_lr_rcp45_PB(m,n) == -99 || maize_Ipsl_cm5a_lr_rcp45_Tave(m,n) == -99 || maize_Ipsl_cm5a_lr_rcp45_Tmax(m,n) == -99 || maize_Ipsl_cm5a_lr_rcp45_Tmin(m,n) == -99 || maize_Ipsl_cm5a_lr_rcp45_ST_original(m,n)==0 || maize_Ipsl_cm5a_lr_rcp45_ST_original(m,n) == 13)
            ...
            maize_Ipsl_cm5a_lr_rcp45_prob_p(m,n)=maize_Ipsl_cm5a_lr_rcp45_prob_positive(m,n);
        end
    end
end
...
cmap(2,:) = [0 0 0]/255;
cmap(3,:) = [135 50 5]/255;
cmap(4,:) = [150 65 15]/255;
cmap(5,:) = [170 90 25]/255;
cmap(6,:) = [195 110 40]/255;


cmap(7,:) = [225 180 90]/255;
cmap(8,:) = [233 205 105]/255;
cmap(9,:)=[240 220 130]/255;
cmap(10,:)=[245 225 160]/255;
cmap(11,:)=[250 235 210]/255;
...
%%
t1=title('(a). Probability of yield gain from CA (+F+WD) in current - maize')
tp1=get(t1,'position')
...
%p2=get(tttt,'position');
%set(tttt,'Position',[p2(1) p2(2)+0.25e6 0.1 ])
t2=title('(b). Probability of yield gain from CA (-F-WD) in current - maize')
tp2=get(t2,'position')
...
%p2=get(tttt,'position');
%set(tttt,'Position',[p2(1) p2(2)+0.25e6 0.1 ])
t3=title('(c). Probability of yield gain from NT+R-SC(+F+WD) in current - maize')
tp3=get(t3,'position')
...
%set(tttt,'Position',[p2(1) p2(2)+0.25e6 0.1 ])
%
t4=title('(d). Probability of yield gain from NT-R-SC(-F-WD) in current - maize')
tp4=get(t4,'position')
